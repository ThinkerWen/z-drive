...
- `app/main.py`：应用入口，负责创建 `FastAPI` 实例、挂载静态资源、注册路由和请求日志中间件。
- 启动时会执行数据库初始化，并创建图库与云盘需要的目录。
...
2. 图片会尝试生成 WebP 预览图，视频和其他文件则按原文件处理。
...
2. 文件管理支持上传、重命名、移动、复制、删除与批量操作。
3. 分享功能会生成分享码，并支持密码和过期时间。
...
- 云盘统计：总文件数、文件夹数、总大小、总空间、可用空间、近期上传和热门访问条目。
